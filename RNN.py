...
class RNN(object):
    ...
    def __init__(self, num_hidden_neurons):
        ...
        # initialize unit values for recurrent connections
    ...
    def feed_forward(self, input_sequence1, input_sequence2):
        ...
        for int1, int2 in zip(input_sequence1, input_sequence2):
            ...
            # calculate signal going into output layer
            z_output = np.dot(x_hidden, self._ff_weights[1])
...
#            calculate output layer output
    ...
    def back_propagate(self, targets, learning_rate):
        ...
        for idx, target in reversed(enum_targets):

            # calculate dedz for output layer
            output = self._xs_output[idx]
            z_output = self._zs_output[idx]
            ...
            z_hidden = self._zs_hidden[idx]
            dedz_hidden = dedx_hidden[1:] * dlogistic_fn(z_hidden)

            # calculate dedws
            dedw_ff_output = np.outer(self._xs_hidden[idx+1], dedz_output)
            dedw_ff_hidden = np.outer(self._xs_input[idx], dedz_hidden)
            # dedw_time_output = np.outer(self._xs_output[timestep], dedz_output)
            dedw_time_hidden = np.outer(self._xs_hidden[idx][1:], dedz_hidden)
        ...
        eff_learning_rate = learning_rate / len(enum_targets)
